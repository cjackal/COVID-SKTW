...
def DataCleaner(config_name, tmp, ver='frozen'):
    ...
    PATH_SCR = os.path.join(homedir, 'tmp', 'preprocessing', tmp)
    ...
    PATH_DEMO = os.path.join(datadir, "aggregate_berkeley.csv")
    PATH_GDP = os.path.join(datadir, "GDP.csv")
    PATH_GEO = os.path.join(datadir, "county_land_areas.csv")
    PATH_MT = os.path.join(datadir, "nyt_us_counties_daily.csv")
    PATH_MB = os.path.join(datadir, "DL-us-mobility-daterow.csv")
    PATH_SS = os.path.join(datadir, "seasonality_stateLevel.csv")
    PATH_POL = os.path.join(datadir, "state_policy.csv")
    ############################################################################
    ...
    gen_frame(config_st, config_ed).to_csv(os.path.join(PATH_SCR, 'frame.csv'), index=False)
    ...
    for i, poptype in enumerate(popularity_type_Male):
        demo['PopRatio'+poptype[3:]] = berkeley[poptype] / (berkeley[poptype]+berkeley[popularity_type_Fmle[i]])
        demo['PopRatio'+poptype[7:]] = berkeley[poptype] / (berkeley['PopTotalMale2017']+berkeley['PopTotalFemale2017'])
    ...
    geo = pd.read_csv(PATH_GEO, usecols=[0,2,3,5])
    ...
    for fips in FIPS_mb:
        ...
        if len(df) != ndays:
            dates_exist = list(df['date'])
            dates_missing = set(dwin).difference(set(dates_exist))
            for dt in dates_missing:
                samedays = [dt + n*oneweek for n in altrange if (dt + n*oneweek) in dates_exist]
                if samedays:
                    m50 = df[df['date']==samedays[0]]['m50'].iloc[0]
                    m50_index = df[df['date']==samedays[0]]['m50_index'].iloc[0]
                else:
                    m50 = df[df['date']==dates_exist[-1]]['m50'].iloc[0]
                    m50_index = df[df['date']==dates_exist[-1]]['m50_index'].iloc[0]
    ...
    for fips in FIPS_demo.difference(FIPS_mb):
        state = str(int(fips[:2]))
        if state in FIPS_mb:
            dummy = mobility[mobility['fips']==state].copy()
            dummy.loc[:, 'fips'] = fips
    ...
    logger.debug('Timeseries features:\n'+', '.join(columns_ts))
    logger.info(f'# Demographic FIPS={len(FIPS_demo)}, '
            f'# Motality FIPS={len(FIPS_mt)}, # Mobility FIPS={len(FIPS_mb)}')
    logger.info(f'First date to be trained: {date_st}, '
                f'Final date to be trained: {date_ed}')
    ...
    data_ctg = []
    counter = 0
    for fips in pbar(sorted(FIPS_demo), desc='Generate training data',
        total=len(FIPS_demo), bar_format='{l_bar}{bar}[{elapsed}<{remaining}]'):
