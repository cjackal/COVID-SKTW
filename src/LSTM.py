...
def train_full(data_ts, data_ctg, target_idx, history_size, target_size,
                step_size=1):
    """
...
def load_Dataset(X_train, C_train, y_train, X_val=None, C_val=None, y_val=None,
                BATCH_SIZE=64, BUFFER_SIZE=100000, cache=False):
    """
...
def LSTM_fit_mult(train_data, val_data=None, hparam=None, monitor=False,
                callbacks=None, ver='frozen', **kwargs):
    """
...
def predict_future(model_qntl, data_ts, data_ctg, scaler_ts, scaler_ctg, history_size, target_idx, FIPS=None, date_ed=None):
    ...
    if (FIPS is None) or (date_ed is None):
        ...
        print('Transform forecast into DataFrame.')
        df_future = []
...
def predict_future_mult(model, data_ts, data_ctg, scaler_ts, scaler_ctg, history_size, target_idx, FIPS=None, date_ed=None):
    ...
    Return:
        pandas Dataframe of the prediction in tidy format.
    """
    X_future = np.asarray([data[-history_size:, :] for data in data_ts])
    C_future = np.asarray(data_ctg)
    ...
    if (FIPS is None) or (date_ed is None):
        ...
        logger.info('Transform forecast into DataFrame.')
        df_future = []
        for i, fips in enumerate(FIPS):
