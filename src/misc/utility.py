...
def get_homedir(verbose=False):
    ...
    homedir = Path(os.path.abspath(getsourcefile(lambda:0))).parents[2]
    if verbose:
        print(homedir)
    return homedir

_homedir = get_homedir()
_FIPS_mapping = os.path.join(_homedir, 'src', 'misc', 'FIPS_mapping.json')

def get_FIPS(path=_FIPS_mapping, reduced=False):
    """
...
def fix_FIPS(df, fipslabel=None, datelabel=None, **kwargs):
    ...
    CAVEAT: May reindex the dataframe.
            ...
            It returns a DataFrame, not in-place.
...
def reformat_prediction(dfs, base='frame.csv', fipslabel='fips',
                        datelabel='date', force_positive=True,
...
def gen_frame(date_st, date_ed, FIPS=_FIPS_mapping):
    import pandas as pd

    with open(FIPS, 'r') as f:
        dic = json.load(f)
